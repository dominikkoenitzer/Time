...
const faqs = [
  {
    ...
    answer: (
      <>
        <p>
          When the page loads, your browser asks our server for the current
          time several times in a row. For each request we know exactly when
          it left and when the answer arrived, so we can estimate the
          difference between your device clock and the server clock — the
          same principle NTP uses to synchronize computers worldwide.
        </p>
        <p>
          We keep the sample with the fastest round trip, because the less
          time the request spent on the network, the less room there is for
          error. The clocks on this site are then corrected by that
          measurement — so what you see is our best estimate of the true
          time, not just a copy of your computer&apos;s clock.
        </p>
      </>
    ),
  },
  {
    ...
    answer: (
      <p>
        ...
        re-synchronization. The measurement is also refreshed when you return
        to the tab after a while.
      </p>
    ),
  },
  {
    question: "What does “Accuracy of synchronization was ±0.023 seconds” mean?",
    answer: (
      <p>
        It is the margin of error of the measurement itself. The server&apos;s
        answer travels over the network, and we can&apos;t know whether it
        spent more time on the way there or on the way back — so the true
        offset could be up to half the round-trip time away from our
        estimate. We state that bound instead of hiding it.
      </p>
    ),
  },
  {
  ...
  },
  {
    ...
    answer: (
      <p>
        ...
        address like{" "}
        <Link href="/europe/zurich" className="text-foreground underline underline-offset-4">
          /europe/zurich
        </Link>{" "}
        or simply{" "}
        <Link href="/tokyo" className="text-foreground underline underline-offset-4">
          /tokyo
        </Link>
        .
      </p>
  ...
  },
  {
    ...
    answer: (
      <p>
        Yes — use the 24H/12H button in the header. The preference is saved
        on your device and applies everywhere: the main clock, the world
        clocks, and search results.
      </p>
    ),
  },
  {
