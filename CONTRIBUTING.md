...
- **Tailwind v4 with CSS-based config.** There is no `tailwind.config.*`; design tokens live in `app/globals.css` under `@theme inline`, declared in both `:root` and `.dark`.- **Time math is client-side `Intl`.** There are no external time APIs and no timezone data files; the server only returns NTP-disciplined time for the sync check. Keep it that way.
