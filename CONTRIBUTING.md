...
- **No `setState` directly inside effects.** The React hooks lint rules forbid it. For hydration/mounted flags and external state, use `useSyncExternalStore` — see the `ThemeToggle` in `components/site-header.tsx` for the established pattern.
