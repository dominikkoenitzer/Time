...
- **ESLint forbids `setState` directly inside effects** (react-hooks rules). For mounted/hydration flags and external state, use `useSyncExternalStore` — see the `ThemeToggle` in `components/site-header.tsx` for the established pattern.
...
- **Routing:** `app/page.tsx` is the immersive kinetic clock (`components/kinetic-clock.tsx`), rendered in the bare root layout — no header or footer. The content pages (`/about`, `/faq`, `/privacy`, `/terms`) live in the `app/(site)` route group, whose layout (`app/(site)/layout.tsx`) adds the shared header/footer chrome. There are no per-timezone pages.
...
- Content pages (about/faq/privacy/terms) share `components/page-shell.tsx` for layout.
