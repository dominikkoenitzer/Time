...
- **Clock sync is the core feature** (`lib/clock-sync.ts`, a module-level external store): NTP-style — samples `/api/time` several times, keeps the lowest-RTT sample, offset is round-trip-compensated, stated accuracy = RTT/2 of the best sample. **Displayed time is corrected by this offset**: `useNow` builds dates from `correctedNowMs()`, so every clock shows server-corrected time, not device time. Watchdogs re-measure when the wall-vs-monotonic clock baseline jumps (manual clock change, sleep/wake) or the tab becomes visible after the result goes stale. `components/sync-status.tsx` is just a renderer of this store.
